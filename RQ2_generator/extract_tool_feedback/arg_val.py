...
def traverse_python_tree(node, results):
    ...
    if node.type == 'function_definition':
        ...
        results.append({"type": "def", "name": function_name, "args": args})
        ...
        if init_method:
            ...
            args = []
        results.append({"type": "def", "name": class_name, "args": args})
    elif node.type == 'call':
        ...
        results.append({"type": "ref", "name": function_name, "args": args})
    return results
...
def traverse_java_tree(node, results):
    ...
    if node.type == 'method_declaration' or node.type == 'constructor_declaration':
        ...
        for param in parameters.children:
            process_args(param, args)
        results.append({"type": "def", "name": method_name, "args": args})
        ...
        for arg in arguments.children:
            if arg.type not in ["(", ")", ","]:
                args.append(arg.text.decode('utf-8').strip())
        results.append({"type": "ref", "name": method_name, "args": args})
        ...
        for arg in arguments.children:
            if arg.type not in ["(", ")", ","]:
                args.append(arg.text.decode('utf-8').strip())
        results.append({"type": "ref", "name": class_name, "args": args})
    return results
...
def traverse_go_tree(node, results):
            ...
            if type_node.text.decode('utf-8').strip()[0].isupper():
                ...
                for param in body_node.children:
                    if param.type not in ["{", "}", ","]:
                        args.append(param.text.decode('utf-8').strip())
                results.append({"type": "ref", "name": struct_name, "args": args})
    ...
    if node.type == 'function_declaration' or node.type == 'method_declaration':
        ...
        results.append({"type": "def", "name": func_name, "args": params})
        ...
        for arg in arguments.children:
            if arg.type not in ["(", ")", ","]:
                args.append(arg.text.decode('utf-8').strip())
        results.append({"type": "ref", "name": func_name, "args": args})
...
def traverse_javascript_tree(node, results):
    ...
    if node.type in ['function_declaration', 'function_expression', 'arrow_function']:
        ...
        try:
            ...
            return results
        results.append({"type": "def", "name": func_name, "args": args})
    elif node.type == 'call_expression':
        ...
        for child in node.children:
            if child.type == 'class_body':
                if child.child_by_field_name('constructor'):
                    ...
                    if constructor_params:
                        ...
                        return results
        results.append({"type": "def", "name": class_name, "args": constructor_args})
    return results
...
def parse_code(code: bytes, lang):
...
def arg_case_diff(before_arg_cases, after_arg_cases):
    """
    Func:
    ...
    Returns:
        result: str | None, the type of change
    """
    ...
    if len(match) == 0:
        ...
        case = match[0]
        change_info = {}
        change_info["type"] = case[0]["type"]
        change_info["name"] = case[0]["name"]
