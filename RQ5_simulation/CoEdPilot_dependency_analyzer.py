...
from transformers import RobertaModel, RobertaTokenizerFast, PreTrainedModel
...
class DependencyAnalyzer(nn.Module, PyTorchModelHubMixin):
    def __init__(self, encoder: PreTrainedModel | None = None,
        ...
        if not encoder:
            encoder: PreTrainedModel = RobertaModel.from_pretrained("microsoft/codebert-base")
        if match_tokenizer:
...
def load_dep_model(args, logger):
    tokenizer = RobertaTokenizerFast.from_pretrained(args.dependency_model_path)
    model = DependencyAnalyzer(match_tokenizer=tokenizer)
    model.load_state_dict(torch.load(os.path.join(args.dependency_model_path,'pytorch_model.bin')))
    model.to(args.device)
    logger.info(f"Successfully loaded dependency model from: {args.dependency_model_path}")
    return model, tokenizer

def cal_dep_score(hunk, file_content, dependency_classifier: DependencyClassifier):
    ...
    corpus_pair = dependency_classifier.construct_corpus_pair(code_window_pairs)
    results = dependency_classifier.batch_gen(corpus_pair)
