...
class Estimator(nn.Module):
    """
    Model to estimate the relevance of prior edits to the current sliding window
    """
    ...
    def forward(self, code_distance: torch.tensor, 
        ...
        dep_score = self.dependency_model(combine_input_ids, combine_attn_masks)[:,-1:]
...
def load_model_estimator(dependency_model, args, logger) -> Tuple[Estimator, RobertaTokenizer]:
    ...
    estimator.load_state_dict(torch.load(args.estimator_model_path))
    logger.info(f"Successfully loaded estimator model from {args.estimator_model_path}")
    return estimator, estimator_tokenizer
   
def load_estimator_data(dataset: List[dict], estimator_tokenizer: RobertaTokenizer, 
        dependency_tokenizer: RobertaTokenizer) -> TensorDataset:
    """
    Func:
    ...
    for sample in dataset:
        ...
        target_code = "".join(sample["sliding_window"])
        prior_edit = sample["prior_edit"].before_edit_window(split_by_line=False) + sample["prior_edit"].after_edit_region(split_by_line=False)
        sep_input = [target_code, prior_edit]
        sep_input = estimator_tokenizer(sep_input, padding="max_length", truncation=True, max_length=512, return_tensors="pt")
        sep_inputs.append(sep_input["input_ids"])
        sep_attn_masks.append(sep_input["attention_mask"])
        
        combine_input = target_code + " </s> " + prior_edit
        combine_input = dependency_tokenizer(combine_input, padding="max_length", truncation=True, max_length=512, return_tensors="pt")
        combine_inputs.append(combine_input["input_ids"])
        combine_attn_masks.append(combine_input["attention_mask"])

        if "label" in sample: # During inference, ground truth label is unknown
    ...
    # print(f"dataset for inference, size: {distance_scores.shape}")
    # print(f"sep_inputs: {sep_inputs.shape}")
    # print(f"sep_attn_masks: {sep_attn_masks.shape}")
    # print(f"combine_inputs: {combine_inputs.shape}")
    # print(f"combine_attn_masks: {combine_attn_masks.shape}")
    
    if len(golds) > 0:
        golds = torch.tensor(golds, dtype=torch.float32)
        dataset = TensorDataset(distance_scores, sep_inputs, sep_attn_masks, combine_inputs, combine_attn_masks, golds)
    else:
...
def evaluate_estimator(model: Estimator, dataloader: DataLoader, mode: str, logger: Union[logging.Logger, None]=None, show_progress: bool = True) -> Union[np.array, Tuple[float, np.array, np.array]]:
    ...
    preds = []
    if show_progress:
        dataloader = tqdm(dataloader, desc="get prior edit estimation",leave=False)
