...
def convert_examples_to_features(examples, tokenizer, args, stage=None):
    ...
    for example_index, example in enumerate(tqdm(examples)):
        ...
        for st, tt in zip(source_tokens, target_tokens):
            if st == tokenizer.mask_token:
                assert tt in ["keep", "replace", "add"]
...
def main():
    ...
    if args.model_type == "codet5":
        codeT5 = model_class.from_pretrained(args.model_name_or_path, config=config) 
        encoder = codeT5.encoder
    else:
        encoder = model_class.from_pretrained(args.model_name_or_path, config=config)
    model=Seq2Seq(encoder=encoder,config=config, args=args,
                  beam_size=args.beam_size,max_length=args.max_target_length,
                  sos_id=tokenizer.cls_token_id,eos_id=tokenizer.sep_token_id,mask_id=tokenizer.mask_token_id,
                  keep_id=tokenizer.convert_tokens_to_ids("keep"),
                  replace_id=tokenizer.convert_tokens_to_ids("replace"),add_id=tokenizer.convert_tokens_to_ids("add"))
